...
void idPlayerView::SingleView( idUserInterface *hud, const renderView_t *view, int renderFlags ) {
	...
	if ( !( RF_GUI_ONLY & renderFlags ) ) {
		...
		//}
		renderView_t eyeView;

		eyeView = *view;
		idVec3 vrHeadOrigin;
		idMat3 vrHeadAxis;
		if(VR_GetHead(vrHeadOrigin, vrHeadAxis)) {
			idVec3 vrDeltaOrigin = vrHeadOrigin;
		...
		{
			renderView_t leftEyeView = eyeView;
		...
		{
			renderView_t rightEyeView = eyeView;
			idVec3 eyeOffset = -idVec3(vrConfig.openVRHalfIPD, vrConfig.openVRHalfIPD, vrConfig.openVRHalfIPD);
			eyeOffset *= eyeView.viewaxis[1];
		...
		eyeView.shaderParms[11] = 0;
		gameRenderWorld->RenderScene(&eyeView, renderFlags | RF_PENUMBRA_MAP);
	}

	//renderSystem->DrawStretchPic(0, 0, renderSystem->GetScreenWidth() / 2, renderSystem->GetScreenHeight(), 0, 0, 1, 1, declManager->FindMaterial("vr/lefteye"));
	//renderSystem->DrawStretchPic(renderSystem->GetScreenWidth() / 2, 0, renderSystem->GetScreenWidth() / 2, renderSystem->GetScreenHeight(), 0, 0, 1, 1, declManager->FindMaterial("vr/righteye"));
